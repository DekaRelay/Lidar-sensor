...
unsigned int edge1; 
int p; 
int q;                                    //Stores the timer count at first edge
...
/*
   ...
   for (x = points; x >= 0; --x){
      ...
      //Check the position of the LiDAR...
...
void horizontalShift(double horizontal){
 ...
 /* PWMCLK = 0x00;                //  Clock B is selected as the clock source
...
double getDistance(void){
 ...
 length = ((double)pulse_width + overflow*65536)/3000;   // Convert the width from counts to milliseconds
